...
function goNext() {
  ...
  if (currSlide == maxSlide) {
    if (userAnswers[currModule] == undefined) {
      $('#myModalAssInst .modal-header h4').html("ASSESSMENT INSTRUCTION");
      $('#myModalAssInst .modal-body p').html("1. Each module assessment has 5 questions. And the Assessments can be taken only once. <ul><li>The scores pertaining to the assessments are important and is part of the overall Pass Criteria.</li></ul>2. Module Assessment has to be taken without any interruptions <ul><li>Planning to windup the learning for the day or take a break, kindly complete the assessments and quit the modules or close the module before taking the Assessments.</li><li>Closing the window during the Assessments, would pass a null score. And will show as '0' for the particular module. And can't be retaken.</li></ul>");
...
function showHtmlContent() {
    ...
    set("cmi.location", String(currModule+'_'+currSlide+'_'+completedModule));    
}
